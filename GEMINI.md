...
* **Subtrava 2.2 (Teto Global de 50% em Criptos e Altcoins):** A exposição combinada em criptoativos voláteis (`BTC`, `ETH`, `SOL`, `LINK`, `BNB`, `ADA`, `NEAR`, `AVAX`, `DOGE`) não pode ultrapassar **50% do patrimônio total consolidado**. Ativos não-cripto (`BRL` Caixa, `USDT` Dólar/FX e `PAXG` Ouro Físico) ficam estritamente de fora desse teto. Se a exposição em criptos/altcoins exceder 50%, **qualquer nova compra de cripto é terminantemente vetada**; vendas e realizações de lucro para BRL, USDT ou PAXG permanecem 100% livres para desestocagem.
